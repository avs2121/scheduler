   ...
   public:
    ...
    std::vector<PCB> process = {
        ...
        for (size_t idx : test_deque)
        {
            ...
            assert_true(iom.containsPID(proc.getPid()),
                        " Original deque, should contain all of the test deques elements");
            assert_equal(proc.getIORemainingTime(),
                         proc.getIOInterval(),
...
class IOProcessTest : public TestFixture
{
   public:
    ...
    std::vector<PCB> process = {
        ...
        assert_equal(iom.getMinRemainingIOTime(),
                     9,
...
void run_io_tests()
{
    ...
    std::cout << "IO test passed with: " << passed << " passed , " << failed << " failed"
              << std::endl;
}