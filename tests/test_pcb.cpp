...
class PCBExecuteTest : public TestFixture
{
   public:
    ...
    void test()
    {
        ...
        assert_equal(p_finish.execute(TEST_TIME),
                     2,
                     " Process [p_finish] should finish with execute time = 2");
        assert_equal(p_finish.getRemainingTime(),
                     0,
...
class PCBAgeTest : public TestFixture
{
   public:
    ...
    void test()
    {
        ...
        assert_equal(p_upgrade.getWaitingTime(),
                     0,
        ...
        assert_equal(p_full_upgraded.getPriority(),
                     1,
                     " Process [p_full_upgraded] should still have priority = 1");
        assert_equal(p_full_upgraded.getWaitingTime(),
                     5 * TEST_TIME,
...
void run_PCB_tests()
{
    ...
    std::cout << "PCB test passed with: " << passed << " passed , " << failed << " failed"
              << std::endl;
}