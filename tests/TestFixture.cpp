...
void TestFixture::trackFile(std::string &filename) {
...
void TestFixture::assert_equal(int actual, int expected,
                               const std::string &message) {
  if (expected != actual) {
    throw std::runtime_error(
        "Assertion failed. Actual: " + std::to_string(actual) +
        " Expected: " + std::to_string(expected) + message);
  }
}

bool TestFixture::fileExists(std::string &filename) {
