...
void IOManager::updateIO()
{
    ...
    {
        if (pcb_ptr->isWaitingIO() && pcb_ptr->isIOBound() && pcb_ptr->getRemainingTime() > 0)
        {
            pcb_ptr->startIO();
        }
    }
...
void IOManager::processIO(int timeslice)
{
    ...
    for (auto &pcb_ptr : IO_queue)
    {
        ...
        {
            pcb_ptr->setIOTime(std::max(0, pcb_ptr->getIORemainingTime() - time_diff));
        }
...
void IOManager::handleIOqueue()
{
    ...
    IO_queue.erase(
        std::remove_if(IO_queue.begin(), IO_queue.end(),
                       [](std::unique_ptr<PCB> &pcb_ptr)
                       { return !(pcb_ptr->isWaitingIO()); }),
        IO_queue.end());
}

const std::vector<std::unique_ptr<PCB>> &IOManager::getFinishedProcesses() const
{
    return finished_IO;
}

bool IOManager::isEmpty() const
{
...
bool IOManager::contains(const PCB *p) const
{
    ...
    return (std::any_of(IO_queue.begin(), IO_queue.end(), [p](const std::unique_ptr<PCB> &pcb_ptr)
                        { return p == pcb_ptr.get(); }));
}
